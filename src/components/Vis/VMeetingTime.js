import { formatMeetingTime } from "./utilities";
import styles from "./VMeetingTime.module.css";

const VMeetingTime = ({ time }) => {
  return (
    <div className={`v ${styles.container}`}>
      ...
      <span className={styles.time}>{formatMeetingTime(time)}</span>
    </div>
  );
};

export default VMeetingTime;
